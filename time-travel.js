...
export function playTimeTravel(container, options, onComplete) {
  ...
  const duration = debug ? 260 : (reduced ? 500 : DURATIONS[intensity]);

  container.innerHTML = `
    <section class="time-travel time-travel--${visualDirection} time-travel--${intensity}${debug ? " time-travel--debug" : ""}" aria-label="Voyage dans le temps">
