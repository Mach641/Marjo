...
export function renderD1PortraitGallery(root, { images, accessibleLabel, placeholderLabel, onClose }) {
  ...
  root.innerHTML = `<section class="d1-portrait-gallery" aria-label="${accessibleLabel}">
    <div class="d1-portrait-gallery__track" tabindex="0" aria-label="${accessibleLabel}">
      ${images.map((image, index) => `<figure class="d1-portrait-gallery__slide${index === 0 && image.immersive ? " d1-portrait-gallery__slide--immersive" : ""}">${image.src ? `<div class="d1-portrait-gallery__scene">${index === 0 && image.introSrc ? `<img class="d1-portrait-gallery__decor" src="${image.introSrc}" alt="" />` : ""}<img class="${index === 0 && image.introSrc ? "d1-portrait-gallery__memory" : ""}" src="${image.src}" alt="${image.alt || accessibleLabel}" /></div>` : `<p>${placeholderLabel}</p>`}${index === 0 && image.immersive ? "" : `<figcaption>${index + 1} / ${images.length}</figcaption>`}</figure>`).join("")}
    </div>
  ...
  const section = root.querySelector('.d1-portrait-gallery');
  ...
  if (memory) {
    Promise.all(Array.from(root.querySelectorAll('.d1-portrait-gallery__scene img')).map(image => image.decode())).then(() => {
      if (disposed) return;
      if (reducedMotion) memory.classList.add('is-revealed');
      else revealTimer = setTimeout(() => { if (!disposed) memory.classList.add('is-revealed'); }, 700);
  ...
  const update = () => {
    ...
    current = next;
    section.classList.toggle('d1-portrait-gallery--immersive', current === 0 && Boolean(images[0]?.immersive));
