...
(async()=>{
 ...
 const cached=await p.evaluate(async()=>{const c=await caches.open('voyage-majorque-v1-4-51-canonical-galleries');return Promise.all(['./road-trip.js?v=1.4.44','./config.js?v=1.4.51','./assets/challenge-5/v1-4-14/map.png','./assets/challenge-5/v1-4-14/camper.png','./assets/finale/v1-4-49/colis.png'].map(async u=>!!await c.match(u)));});
